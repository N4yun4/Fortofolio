...
const REVEAL_CONTENT = [
  {
    upper: [
      "Name: Nayun", 
      "Role: Newbie AMV Maker & Junior Stock Market Analyst",
      "Risk comes from not knowing what you are doing.(Warren Buffett)",
    ],
    lower: "Low Cortisol people 🍃",
  },
  {
    upper: [
      "Specializes in AMV editing with After Effect",
      "Tracks stock market trends and technical analysis daily.",
      "just an ordinary human who likes playing games",
    ],
    lower: "creative × analytical",
  },
  {
    upper: [
      "Just someone who loves editing photos and videos since childhood..",
      "Stock analysis: reading charts and fundamental stock like reading a story.",
      "Rule No. 1: Never lose money. Rule No. 2: Never forget rule No. 1.(Warren Buffett)",
    ],
    lower: "always learning, always improving, always chill.",
  },
];

const ROLES = [
  { text: "NAYUN",  color: "#e8c100", bg: "rgba(232,193,0,0.12)", border: "rgba(232,193,0,0.5)" },
  { text: "SKILL",  color: "#4a8fff", bg: "rgba(74,143,255,0.12)", border: "rgba(74,143,255,0.5)" },
  { text: "STORY",  color: "#4a8fff", bg: "rgba(74,143,255,0.12)", border: "rgba(74,143,255,0.5)" },
];
    ...
    <div id="menu-screen">
      ...
      {revealed && (
        <div key={`nav-${active}`} className="sc-right-nav">
          <span className="sc-nav-arrow left" onClick={(e) => { e.stopPropagation(); setActive(i => Math.max(0, i - 1)); }}>◄</span>
          <span className="sc-nav-btn" onClick={(e) => { e.stopPropagation(); setActive(i => Math.max(0, i - 1)); }}>LB</span>
          <span className="sc-nav-dot" />
          <span className="sc-nav-btn" onClick={(e) => { e.stopPropagation(); setActive(i => Math.min(ITEMS.length - 1, i + 1)); }}>RB</span>
          <span className="sc-nav-arrow right" onClick={(e) => { e.stopPropagation(); setActive(i => Math.min(ITEMS.length - 1, i + 1)); }}>►</span>
        </div>
      )}
      {revealed && (
